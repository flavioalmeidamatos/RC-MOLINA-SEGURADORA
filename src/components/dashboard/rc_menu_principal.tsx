            ...
            <div className="overflow-y-auto custom-scrollbar p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  { name: "PORTO SEGURO", url: "https://corretor.portoseguro.com.br/portal/site/corretoronline/template.LOGIN/", logo: "/portais/porto_seguro.webp", executeScript: "(function(){let attempts=0;let clicked=false;let interval=setInterval(function(){if(!clicked){let btns=Array.from(document.querySelectorAll('button, a, div[role=\"button\"], span'));let btn=btns.find(b=>b.textContent&&b.textContent.toUpperCase().includes('ACESSAR O CORRETOR ONLINE'));if(btn){btn.click();clicked=true;}}let user=document.querySelector('input[type=\"text\"], input[type=\"email\"], input[name*=\"login\"], input[name*=\"user\"], input[name*=\"usuario\"], input[name*=\"cpf\"], #login, #usuario, #login_usuario, #cpf, [placeholder*=\"CPF\"], [placeholder*=\"Usuário\"]');let pass=document.querySelector('input[type=\"password\"], input[name*=\"senha\"], input[name*=\"pass\"], #senha, #password, #login_senha, [placeholder*=\"Senha\"]');if(user&&pass&&user.offsetParent!==null){let setValue=function(el,value){let setter=Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype,'value');if(setter&&setter.set){setter.set.call(el,value);}else{el.value=value;}el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));};setValue(user,'77915445715');setValue(pass,'Benj@min88');clearInterval(interval);}attempts++;if(attempts>45){clearInterval(interval);}},1000);})();" },
