...
export const SCR_MENUPRINCIPAL: React.FC<DashboardProps> = ({ session, perfil }) => {
    const navigate = useNavigate();
    const userName = perfil?.nome_completo || session?.user?.email?.split('@')[0] || 'Nome do Usuário';
    ...
    const menuItems = [
        ...
        { title: 'Indicações', icon: User, hasArrow: true },
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <Icon size={18} className={activeMenu === item.title ? 'text-[#b58c2a]' : 'text-gray-500'} />
                                    <span className="text-sm">{item.title}</span>
                                </div>
                                {item.hasArrow && <ChevronLeft size={16} className="text-gray-600" />}
                            </div>
                        );
                    })}
                </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="w-1/3 bg-[#a2812a] group-hover:bg-[#8f7124] transition-colors flex items-center justify-center text-white">
                                        <Icon size={34} strokeWidth={1.5} className="group-hover:scale-110 transition-transform duration-300" />
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="w-full lg:w-80 flex-shrink-0 bg-white shadow-sm flex flex-col border border-gray-100 rounded-sm overflow-hidden h-fit max-h-full">
                        ...
                        <div className="px-4 py-3 flex border-b border-gray-100 items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-1 text-gray-400">
                                <input type="text" placeholder="Hora" className="w-10 text-center outline-none bg-transparent" />
                                <span>:</span>
                                <input type="text" placeholder="Min." className="w-10 text-center outline-none bg-transparent" />
                            </div>
                        </div>
                            ...
                            {agendaItems.map((item, idx) => (
                                ...
                                >
                                    <div className="flex flex-col items-center gap-2 mt-1">
                                        <User fill="currentColor" size={14} className={item.highlight ? 'text-red-500' : 'text-gray-500'} />
                                        <Phone fill="currentColor" size={14} className={item.highlight ? 'text-red-500' : 'text-gray-500'} />
                                    </div>
                                    <div className="flex-1 space-y-1">
                                        <div className={`text-sm font-semibold ${item.highlight ? 'text-red-500' : 'text-[#555]'}`}>
                                            {item.name}
                                        </div>
                                        <div className={`text-xs ${item.highlight ? 'text-red-500' : 'text-gray-500'}`}>
                                            {item.phone}
                                        </div>
                                        <div className="flex gap-2 items-center text-xs text-gray-500">
                                            <span className={item.highlight ? 'text-red-400 font-medium' : ''}>{item.time}</span>
                                            <span className={item.highlight ? 'text-red-400' : ''}>{item.status}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
