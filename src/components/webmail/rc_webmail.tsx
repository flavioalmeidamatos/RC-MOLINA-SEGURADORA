...
import { detectEmailProvider } from '../../lib/validacoes';
...
type RCWebmailProps = {
  ...
  redirectMessageId?: string | null;
  onClearRedirectMessageId?: () => void;
  lastInboxUpdateTimestamp?: number;
};

export function RCWebmail({
  userId,
  userEmail,
  redirectMessageId,
  onClearRedirectMessageId,
  lastInboxUpdateTimestamp,
}: RCWebmailProps) {
  ...
  useEffect(() => {
    if (redirectMessageId) {
      const messageId = redirectMessageId;
      openFolder('inbox');
  ...
  useEffect(() => {
    if (lastInboxUpdateTimestamp && lastInboxUpdateTimestamp > 0 && activeFolder === 'inbox') {
      void loadMessages('inbox');
    }
  }, [lastInboxUpdateTimestamp, activeFolder]);
