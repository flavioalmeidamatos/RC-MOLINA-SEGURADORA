...
export const WhatsAppQrPanel: React.FC = () => {
  ...
  const loadQrCode = async (isManualRefresh = false) => {
    if (isManualRefresh) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      const data = (await response.json()) as WhatsAppQrApiResponse;

      if (!response.ok || !data.qrCode) {
        throw new Error(data.error || "Nao foi possivel carregar o QR Code.");
      }
      ...
      const message =
        loadError instanceof Error ? loadError.message : "Erro inesperado ao buscar o QR Code.";
      setError(message);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  return (
    <div className="flex flex-1 overflow-y-auto bg-[#eef3f7] p-4 sm:p-6 md:p-8">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 lg:grid lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,420px)] lg:items-stretch lg:gap-6">
        <section className="rounded-[28px] border border-[#d7e2ea] bg-white p-5 shadow-sm sm:p-6 lg:p-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[#516072] sm:text-base">
            Abra o WhatsApp no celular, toque em aparelhos conectados e escaneie o QR Code exibido
            ao lado. Quando o codigo expirar, a atualizacao manual ou automatica traz uma nova
            imagem.
          </p>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-[#dce6ed] bg-[#f7fafc] p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-[#334155]">
                Mire a camera no codigo. O QR fica grande no mobile e ainda mais confortavel no
                desktop.
              </p>
            </div>
            <div className="rounded-2xl border border-[#dce6ed] bg-[#f7fafc] p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-[#334155]">
                Depois da leitura, a conta permanece conectada e voce segue no atendimento sem sair
                do sistema.
              </p>
            </div>
          </div>
        ...
        <aside className="rounded-[28px] border border-[#d7e2ea] bg-white p-5 shadow-sm sm:p-6 lg:p-7">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-2 text-lg font-semibold text-[#0c1826]">Leia com o celular</h3>
            </div>
            <button
              type="button"
          ...
          <div className="mt-5 rounded-[24px] border border-dashed border-[#cfe0d5] bg-[#f7fbf8] p-4 sm:p-5">
            {loading ? (
              <div className="flex min-h-[300px] flex-col items-center justify-center gap-3 text-center text-[#516072]">
                ...
                <p className="text-sm font-medium">Buscando o QR Code do WhatsApp...</p>
              </div>
            ) : error ? (
          ...
          <div className="mt-4 rounded-2xl border border-[#dce6ed] bg-[#f8fbfd] p-4">
            <div className="flex items-start gap-3">
              <CheckCircle2 className="mt-0.5 h-5 w-5 flex-shrink-0 text-[#128C7E]" />
              <p className="text-sm leading-6 text-[#334155]">
                O QR Code e recarregado automaticamente a cada 45 segundos para reduzir falhas por
                expiracao. Se precisar, voce tambem pode atualizar manualmente.
              </p>
            </div>
          </div>
        </aside>
